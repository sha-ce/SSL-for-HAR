...
def weighted_epoch_sample(data_with_std, num_sample=400, label=False, dic=None, group=None):
    """
    Weighted sample the windows that have most motion
    Args:
        data_with_std (np_array) of shape N x 3 x 31:
    ...
    if label:
        ...
        sampled_label = np.zeros([num_sample, ])
        for ii in range(num_sample):
            idx = sample_ides[ii]
            # current_sample = ori_data[idx, :, :]
            sampled_data[ii, :] = ori_data[idx, :, :]
            sampled_label[ii] = dic[group][idx]
...
class SSL_dataset:
    def __init__(
        ...
        self.cfg = cfg
        self.is_epoch_data = cfg.runtime.is_epoch_data
...
class SSL_learge_dataset:
    def __init__(
        ...
        self.label = label
        self.dic = dic
        size_and_data = load_data(self.file_list)
        self.len = size_and_data[0]
        self.data = size_and_data[1]
...
def load_data(file_list):
    ...
    for path in file_list:
        x = np.load(path, allow_pickle=True)[:,:,:-1]
        size+=len(x)
        xs.append(torch.from_numpy(x))
    return size, torch.cat(xs, dim=0)
